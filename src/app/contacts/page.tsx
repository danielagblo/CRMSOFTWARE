  ...
  const handlePushToLeads = async (contact: ContactEntry) => {
    ...
    try {
      ...
      const nextContacts = contacts.map((item) =>
        item.id === contact.id
          ? { ...item, pushedLeadId: 'id' in data ? data.id : undefined, pushedAt: new Date().toISOString() }
          : item
      )
      persistContacts(nextContacts)
      alert('Contact pushed to leads successfully.')
    } catch (error) {
                ...
                <div key={contact.id} className="rounded-xl border border-gray-200 bg-white p-3 shadow-sm">
                  ...
                  <div className="mt-3 flex gap-2">
                    <button
                      ...
                      disabled={!!contact.pushedAt || isPushingId === contact.id}
