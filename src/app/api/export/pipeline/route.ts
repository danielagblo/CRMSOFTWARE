...
export async function GET(request: NextRequest) {
  try {
    ...
    const selectedLeadIds = (leadIdsParam || '')
      .split(',')
      .map((id) => id.trim())
      .filter(Boolean)
    ...
    const docHtml = `
      <html>
        ...
        <body>
          <h1>CRM Pipeline Export Report</h1>
          <p class="subtitle">Exported: ${htmlEscape(new Date().toISOString())} | Total Leads: ${htmlEscape(leads.length)}</p>
          ${leadSections || '<p>No leads found for export.</p>'}
        </body>
      </html>
    `
    ...
    return new NextResponse(docHtml, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="${filenamePrefix}-${timestamp}.doc"`
      }
    })
  } catch (error) {
    return NextResponse.json({ error: 'Failed to export pipeline data' }, { status: 500 })
  }
}
